...
class CustomUserSerializer(serializers.ModelSerializer):
    ...
    character_image = serializers.CharField(write_only=True)

    class Meta:
        model = CustomUser
        fields = [
            'email', 'username', 'password', 'date_of_birth', 'height', 'weight',
            'terms_and_conditions', 'privacy_policy', 'marketing', 'character_image',
        ]
    ...
    def validate(self, value):
        ...
        if not value['character_image']:
            raise serializers.ValidationError({'character_image': 'Invalid character image'})
        return value
